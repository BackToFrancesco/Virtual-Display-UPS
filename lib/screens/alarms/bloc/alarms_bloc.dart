...
class AlarmsBloc extends Bloc<AlarmsEvent, AlarmsState> {
  ...
  void _onInit(Init event, Emitter<AlarmsState> emit) {
    emit(state.copyWith(alarms: _modbusDataRepository.getAlarms()));
  }

  void _onDataChanged(DataChanged event, Emitter<AlarmsState> emit) {
    emit(state.copyWith(alarms: _modbusDataRepository.getAlarms()));
  }

  void _onUpsStatusChanged(UpsStatusChanged event, Emitter<AlarmsState> emit) {
